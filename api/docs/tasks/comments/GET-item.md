...
| Service Method | GET |
| Description | Возвращает комментарий задачи |
| Return Type | JSON |
...
| comment_id | STRING | 36 | Yes | Уникальный идентификатор комментария |
...
- **Authorization**: пользователь может получить комментарий задачи, к которой у него есть доступ.
...
1. Клиент отправляет `GET /api/v1/tasks/{task_id}/comments/{comment_id}`.
...
```json
{
  ...
  "data": {
    ...
    "text": "Нужно добавить валидацию входных данных",
...
```http
GET /api/v1/tasks/invalid-task-id/comments/invalid-comment-id
Authorization: Bearer <access_token>
```
...
```http
GET /api/v1/tasks/123e4567-e89b-12d3-a456-426614174000/comments/99999999-9999-9999-9999-999999999999
Authorization: Bearer <access_token>
```
