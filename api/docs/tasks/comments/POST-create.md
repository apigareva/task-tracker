...
| Service Method | POST |
| Description | Создает комментарий к задаче |
| Return Type | JSON |
...
- **Authorization**: пользователь может создать комментарий к задаче, к которой у него есть доступ.
...
1. Клиент отправляет `POST /api/v1/tasks/{task_id}/comments`.
