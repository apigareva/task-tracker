# Получить файл задачи

|  |  |
|------|------|
| URL | /api/v1/tasks/{task_id}/files/{file_id} |
| Service Method | GET |
| Description | Возвращает метаданные файла задачи |
| Return Type | JSON |
...
- **Authorization**: пользователь может получить файл задачи, к которой у него есть доступ.
...
1. Клиент отправляет `GET /api/v1/tasks/{task_id}/files/{file_id}`.
...
```http
GET /api/v1/tasks/{task_id}/files/{file_id}
...
```http
GET /api/v1/tasks/invalid-task-id/files/invalid-file-id
Authorization: Bearer <access_token>
```
...
```http
GET /api/v1/tasks/123e4567-e89b-12d3-a456-426614174000/files/99999999-9999-9999-9999-999999999999
Authorization: Bearer <access_token>
```
