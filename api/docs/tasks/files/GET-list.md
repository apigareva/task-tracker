...
| Service Method | GET |
| Description | Возвращает список файлов, прикрепленных к задаче |
| Return Type | JSON |
...
- **Authorization**: пользователь может получить файлы задачи, к которой у него есть доступ.
...
1. Клиент отправляет `GET /api/v1/tasks/{task_id}/files`.
...
```http
GET /api/v1/tasks/invalid-task-id/files
Authorization: Bearer <access_token>
```
...
```http
GET /api/v1/tasks/99999999-9999-9999-9999-999999999999/files
Authorization: Bearer <access_token>
```
