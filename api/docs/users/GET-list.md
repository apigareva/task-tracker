...
| status | STRING | 100 | No | Фильтр по статусу пользователя |
| email | STRING | 320 | No | Фильтр по email пользователя |
...
1. Клиент отправляет `GET /api/v1/users`.
2. Сервер валидирует Bearer access-токен и административные права.
3. Сервер получает список пользователей из `users` с учетом фильтров.
