...
- **Authorization**: пользователь может обновить собственный профиль или администратор может обновить данные любого пользователя.
- **Session policy**: если статус пользователя меняется на `BLOCKED`, все активные refresh-сессии пользователя отзываются.
...
1. Клиент отправляет `PUT /api/v1/users/{user_id}`.
2. Сервер валидирует Bearer access-токен, входные параметры и права доступа.
3. Сервер обновляет поля пользователя в `users`.
4. Если `status` изменен на `BLOCKED`, сервер отзывает активные сессии пользователя в `auth_sessions`, заполняя `revoked_at` и `revoked_reason = blocked`.
5. Сервер формирует ответ с обновленными данными пользователя без `password_hash` и security-полей.
...
| status | `users.status` |
| status = BLOCKED | `auth_sessions.revoked_at`, `auth_sessions.revoked_reason` |
...
```http
PUT /api/v1/users/invalid-user-id
Authorization: Bearer <access_token>
Content-Type: application/json
