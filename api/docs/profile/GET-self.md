...
- **Authorization**: пользователь получает только собственный профиль.
...
1. Клиент отправляет `GET /api/v1/profile`.
2. Сервер валидирует Bearer access-токен и определяет пользователя по JWT claim `sub`.
3. Сервер получает профиль из `users` по `users.id = sub`.
...
### 9.3 Response 429 Too Many Requests
...
### 9.4 Response 500 Internal Server Error
...
### 9.5 Response 503 Service Unavailable
