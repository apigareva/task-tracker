...
| Description | Удаляет проект по уникальному идентификатору |
...
- **Authorization**: пользователь может удалить только проект, к которому у него есть доступ.
...
1. Клиент отправляет `DELETE /api/v1/projects/{project_id}`.
