...
| Service Method | GET |
| Description | Возвращает данные проекта по уникальному идентификатору |
| Return Type | JSON |
...
- **Authorization**: пользователь может получить только проект, к которому у него есть доступ.
...
1. Клиент отправляет `GET /api/v1/projects/{project_id}`.
