...
| URL | /api/v1/teams |
| Service Method | GET |
| Description | Возвращает список команд с фильтрами и пагинацией |
| Return Type | JSON |
...
- **Authorization**: пользователь видит только команд, к которым у него есть доступ.
...
1. Клиент отправляет `GET /api/v1/teams`.
