...
| Name | Type | Max length | Required | Description |
|------|------|------|------|------|
| member_id | STRING | 36 | Yes | UUID пользователя |
| role_id | STRING | 36 | Yes | UUID роли |
...
- **Authorization**: пользователь может добавлять участников только в команду, которой управляет.
...
1. Клиент отправляет `POST /api/v1/teams/{team_id}/members`.
...
| 404 | Ресурс не найден | false | `not_found` | "Team, user or role not found" |
...
```json
{
  "success": false,
  "data": null,
  "errorMessage": "Team, user or role not found"
}
```
