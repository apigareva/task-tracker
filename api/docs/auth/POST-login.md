# Войти в систему трекинга задач
...
|------|------|------|------|------|
| email | STRING | 320 | Yes | Email пользователя |
...
- **Rate limiting**: ограничение на количество запросов — не более 10 запросов в минуту с одного IP или токена.
- **Data Encryption**: передача данных только по HTTPS (TLS 1.2+).

## 5. How the service works

1. Клиент отправляет `POST /api/v1/auth/login`.
2. Сервер валидирует входные параметры и находит пользователя по `users.email`.
3. Сервер проверяет `users.password_hash`, статус пользователя, `users.locked_until` и счетчик `users.failed_login_attempts`.
4. При успешной аутентификации сервер создает access JWT и refresh-токен.
5. Сервер сохраняет хэш refresh-токена и `jti` JWT в `auth_sessions`, обновляет `users.last_login_at` и сбрасывает `users.failed_login_attempts`.
6. Сервер возвращает пару токенов и данные пользователя.
...
|------|------|
| email | `users.email` |
| password | Проверяется с `users.password_hash` |
| generated refresh_token | `auth_sessions.refresh_token_hash` |
| generated JWT `jti` | `auth_sessions.jwt_id` |
| User-Agent header | `auth_sessions.user_agent` |
| client IP | `auth_sessions.ip_address` |
...
| Table | Purpose |
|------|------|
| `users` | Поиск пользователя по email, проверка статуса, блокировок и хэша пароля |
| `auth_sessions` | Создание пользовательской сессии и хранение хэша refresh-токена |
...
| 401 | Не авторизован | false | `unauthorized` | "Invalid email or password" |
